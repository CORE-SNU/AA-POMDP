...
class AAFIB(OfflineSolver):
    def __init__(self, 
        ...
        Initialization
        """
        super(AAFIB, self).__init__(pomdp, eps)
    ...
    def solve_AA(self):
        ...
        while time_step < self.max_iter:
            ...
            if (self.safeguard == "safe_local"):
                ...
                # approx_local_conv_2 = opt_gain_2 + m * g_max
        ...
        if self.max_type == "mellowmax" and not self.sample:
            for a in range(self.a_num):
                ...
                for o in range(self.o_num):
                    q = np.array([self.pomdp.OT[o][a].dot(Q[i,:]) for i in range(Q.shape[0])])
                    trick = np.max(q)
                    v += (1/beta) * np.log( np.sum( np.exp( beta * (q - trick) ), axis = 0) / self.a_num ) + trick
                Q_value[a, :] = self.rewards[a, :] + self.pomdp.discount * v
...
class AAQMDP(AAFIB):
    ...
    def updateQValueFromValues(self):
        ...
        # Hard max
        if self.max_type == "standard" and not self.sample:
            v_next = np.zeros([self.s_num])
            # For extreme case (Random policy)
            # for s in range(self.s_num):
            #     v_next[s] = Q[self.random_policy[s],s]
            for a in range(self.a_num):
                v = np.max(np.array([self.pomdp.tran[a].dot(Q[i,:]) for i in range(Q.shape[0])]), axis = 0)
                # Random policy (legacy)
                # v = np.mean(np.array([self.pomdp.tran[a].dot(Q[i,:]) for i in range(Q.shape[0])]), axis = 0)
                # # v = self.pomdp.tran[a].dot(v_next)
                Q_value[a, :] = self.rewards[a, :] + self.pomdp.discount * v
        
        if self.max_type == "standard" and self.sample:
            for a in range(self.a_num):
                v = np.max(np.array([self.T_hat[a].dot(Q[i,:]) for i in range(Q.shape[0])]), axis = 0)         
                Q_value[a, :] = self.R_hat[a, :] + self.pomdp.discount * v

        # logsumexp
        if self.max_type == "logsumexp" and not self.sample:
            for a in range(self.a_num):
                q = np.array([self.pomdp.tran[a].dot(Q[i,:]) for i in range(Q.shape[0])])
                trick = np.max(q)
                v = (1/beta) * np.log( np.sum( np.exp( beta * (q - trick) , dtype=np.float128), axis = 0) ) + trick
                Q_value[a, :] = self.rewards[a, :] + self.pomdp.discount * v
        ...
        if self.max_type == "logsumexp" and self.sample:
            for a in range(self.a_num):
                q = np.array([self.T_hat[a].dot(Q[i,:]) for i in range(Q.shape[0])])
                trick = np.max(q)
                v = (1/beta) * np.log( np.sum( np.exp( beta * (q - trick) , dtype=np.float128), axis = 0) ) + trick
                Q_value[a, :] = self.R_hat[a, :] + self.pomdp.discount * v
