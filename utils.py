...
def simulate(pomdp, sample_size):
    ...
    # w/o multiprocessing, process slows down largely!
    # result = {(s,a):[] for s in range(s_num) for a in range(a_num)}
    # for s in range(s_num):
    #     for a in range(a_num):
    #         result[(s,a)].extend(environment.sample((s, a, sample_size)))

    T_hat = {a: dok_matrix((s_num, s_num)) for a in range(a_num)}
